...
(async () => {
  ...
  while (true) {
    ...
    if (hashes.length >= BATCH_SIZE) {
      log('log hashes', hashes.length);
      // const r = await processNewHashBatch(hashes, contract);
      await redisClient.xAdd('x1:log_batches', '*', { type: '0', hashes: JSON.stringify(hashes) });
      log('log hashes batched');
      // clear buffer
      hashes.length = 0;
    }
    if (xunis.length >= BATCH_SIZE) {
      log('log xunis', xunis.length)
      // const r = await processHashBatch(xunis, contract, wallet.address);
      await redisClient.xAdd('x1:log_batches', '*', { type: '1', hashes: JSON.stringify(xunis) });
      log('log xunis batched');
      // clear buffer
      xunis.length = 0;
    }
  }
